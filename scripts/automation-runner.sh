...
run_job_unlocked() {
  ...
  case "${job}" in
    db)
      timeout_seconds="${AV_AUTOMATION_DB_TIMEOUT_SECONDS:-3300}"
      run_with_timeout "${timeout_seconds}" "${script_dir}/update-db.sh" --color never
      exit_code=$?
      ;;
    pkg-origin)
      timeout_seconds="${AV_AUTOMATION_PKG_ORIGIN_TIMEOUT_SECONDS:-21600}"
