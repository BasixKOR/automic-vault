...
const OPENHUE_CLI_ISOTOPE_SCRIPT: &str = include_str!("../scripts/build-openhue-cli-isotope.sh");
const PLUMBER_ISOTOPE_SCRIPT: &str = include_str!("../scripts/build-plumber-isotope.sh");
...
fn release_workflow_binds_the_dmg_to_reviewed_source() {
    ...
    assert!(OXIDE_ISOTOPE_SCRIPT.contains("isotopes/oxide.commit"));
    ...
    assert!(RAILWAY_ISOTOPE_SCRIPT.contains("isotopes/railway.source-sha256"));
    assert!(RAILWAY_ISOTOPE_SCRIPT.contains("isotopes/railway.commit"));
    assert!(RAILWAY_ISOTOPE_SCRIPT.contains("patch --batch --fuzz=0"));
    assert!(RAILWAY_ISOTOPE_SCRIPT.contains("cargo +\"$TOOLCHAIN\" build --locked"));
    assert!(RAILWAY_ISOTOPE_SCRIPT.contains("codesign --verify --strict"));
    assert!(ORDERCLI_ISOTOPE_SCRIPT.contains("isotopes/ordercli.source-sha256"));
    assert!(ORDERCLI_ISOTOPE_SCRIPT.contains("isotopes/ordercli.commit"));
    assert!(ORDERCLI_ISOTOPE_SCRIPT.contains("patch --batch --fuzz=0"));
    ...
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("isotopes/openhue-cli.source-sha256"));
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("isotopes/openhue-cli.commit"));
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("patch --batch --fuzz=0"));
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("GOTOOLCHAIN=\"$TOOLCHAIN\""));
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("vcs.revision=$COMMIT"));
    assert!(OPENHUE_CLI_ISOTOPE_SCRIPT.contains("codesign --verify --strict"));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("isotopes/plumber.source-sha256"));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("isotopes/plumber.commit"));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("patch --batch --fuzz=0"));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("GOTOOLCHAIN=\"$TOOLCHAIN\""));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("vcs.revision=$COMMIT"));
    assert!(PLUMBER_ISOTOPE_SCRIPT.contains("codesign --verify --strict"));
