...
#[test]
fn av_harden_aws_without_confirmation_cancels() {
    let home = temp_home("harden-aws-cancel");
    let keychain = home.join("keychain");
    let aws = home.join(".aws");
    fs::create_dir_all(&keychain).unwrap();
    fs::create_dir_all(&aws).unwrap();
    ...
    assert!(!aws.join("config").exists());
    assert!(!keychain.join("AWS_ACCESS_KEY_ID").exists());
