...
#[allow(dead_code)]
mod uv_post_install_e2e {
    include!("../data/radioisotopes/uv/post-install.rs");

    use std::process::Command;

    fn write_executable_for_e2e(path: &Path, contents: &[u8]) {
    ...
    #[test]
    fn generated_uv_wrapper_runs_through_real_av_without_optional_credentials() {
        if unsafe { libc::geteuid() } == 0 {
            return;
        }

        let temp = tempfile::tempdir().unwrap();
        let original = temp.path().join("uv.av-orig");
        let wrapper = temp.path().join("uv");
        let credentials_key = format!("UV_CREDENTIALS_TOML_E2E_{}", std::process::id());

        write_executable_for_e2e(
            &original,
            br#"#!/bin/sh
set -eu
test -d "$UV_CREDENTIALS_DIR"
test -f "$UV_CREDENTIALS_DIR/credentials.toml"
printf "argc:%s\n" "$#"
printf "arg1:%s\n" "${1-}"
printf "credentials:%s\n" "$(cat "$UV_CREDENTIALS_DIR/credentials.toml")"
"#,
        );
        write_executable_for_e2e(
            &wrapper,
            uv_wrapper_script_with_key_and_av(
                &original,
