...
Do not regenerate, edit, or rely on `www/pkg/**`, localized `*/pkg/**`, or
`www/pagefind/**` as the live package catalog. Those paths are legacy generated
artifacts and are excluded from website syncs.
...
`/pkg/search.json` are Atlas origin routes, not S3 static pages.
...
- Keep static assets under `assets/` unless the existing page pattern uses a
  different location.
- Do not add package search or Pagefind output to this directory. Package search
  is handled by `av-web` at `/pkg/search.json`.
...
- Keep SEO-facing alternate formats aligned with the canonical page content when
  ...
  package-origin generator or `av-web`, not in `www/pkg`.
