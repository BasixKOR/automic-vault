pub(crate) use super::script_resolution::validate_root_controlled_path;
use super::script_resolution::{
    self, ScriptExecutionScope, path_to_display_string, validate_parent_directories,
    validate_regular_target, validate_target_root_installation,
};
#[cfg(test)]
use super::script_resolution::{
    ScriptResolution, interpreter_option_takes_value, interpreter_script,
    interpreter_script_operand, interpreter_script_path_for_display, is_script_interpreter,
    resolve_script_operand, sha256_file, validate_directory_mode, validate_target_file_metadata,
};
use super::*;
...
fn always_allow_scope(
    ...
    let script = script_resolution::script_execution_scope(
        executable_path,
    ...
    Ok(IsotopeAlwaysAllowScope {
        executable_path: script.executable_path,
...
fn script_path_for_display(
    ...
    let scope = always_allow_scope.map(|scope| ScriptExecutionScope {
        executable_path: scope.executable_path.clone(),
        script_path: scope.script_path.clone(),
        script_sha256: scope.script_sha256.clone(),
    });
    script_resolution::script_path_for_display(executable_path, scope.as_ref(), args)
}

#[cfg(test)]
type IsotopeAlwaysAllowScript = ScriptResolution;
...
fn interpreter_script_for_always_allow(
    ...
    interpreter_script(executable_path, args)
}
