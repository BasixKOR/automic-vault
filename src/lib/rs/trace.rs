...
fn run_trace_request_with_progress(
    ...
    let safety_rating = trace_safety_rating_for_command(&request.command, &steps);
    Ok(TraceReport {
...
If a `docker run` command uses a writable host bind mount such as `-v
/etc:/host-etc:rw` or `--mount type=bind,source=/etc,target=/host-etc`, and the
container command writes through that mounted container path, report the host
path being changed rather than only the container path.
...
fn trace_safety_rating_for_command(command: &str, steps: &[TraceStep]) -> TraceSafetyRating {
    let mut rating = trace_safety_rating(steps);
    let mut level = trace_safety_level_from_rating(&rating);

    if trace_command_writes_privileged_host_path_through_docker(command) {
        push_trace_safety_signal(
            &mut level,
            &mut rating.reasons,
            TraceSafetyLevel::Danger,
            "writes host files through Docker bind mount",
        );
    }

    rating.level = level.as_str().to_string();
    rating
}

fn trace_safety_level_from_rating(rating: &TraceSafetyRating) -> TraceSafetyLevel {
    match rating.level.as_str() {
        "danger" => TraceSafetyLevel::Danger,
        "moderate" => TraceSafetyLevel::Moderate,
        _ => TraceSafetyLevel::Safe,
    }
}
...
fn trace_command_writes_privileged_host_path_through_docker(command: &str) -> bool {
    let Some(tokens) = shell_words_for_trace(command) else {
        return false;
    };

    for index in 0..tokens.len() {
        if trace_command_basename(&tokens[index]) != "docker" {
            continue;
        }
        if docker_run_tokens_write_privileged_host_mount(&tokens[index + 1..]) {
            return true;
        }
    }
...
fn docker_run_tokens_write_privileged_host_mount(tokens: &[String]) -> bool {
    let Some(run_start) = docker_run_args_start(tokens) else {
        return false;
    };
    let args = &tokens[run_start..];
    let mut mounts = Vec::new();
    let mut index = 0;

    while index < args.len() {
        let token = args[index].as_str();
        if trace_shell_token_is_command_separator(token) {
            return false;
        }
        if token == "--" {
            index += 1;
        ...
        if matches!(token, "-v" | "--volume") {
            if let Some(spec) = args.get(index + 1) {
                if let Some(mount) = parse_docker_volume_mount(spec) {
                    mounts.push(mount);
                }
            }
            index += 2;
        ...
        if let Some(spec) = token.strip_prefix("--volume=") {
            if let Some(mount) = parse_docker_volume_mount(spec) {
                mounts.push(mount);
            }
            index += 1;
        ...
        if let Some(spec) = token.strip_prefix("-v").filter(|spec| !spec.is_empty()) {
            if let Some(mount) = parse_docker_volume_mount(spec) {
                mounts.push(mount);
            }
            index += 1;
        ...
        if token == "--mount" {
            if let Some(spec) = args.get(index + 1) {
                if let Some(mount) = parse_docker_key_value_mount(spec) {
                    mounts.push(mount);
                }
            }
            index += 2;
        ...
        index += if docker_run_option_takes_value(token) {
            2
        } else {
            1
        };
    ...
    index += 1;
    let container_command = args[index..]
        .iter()
        .take_while(|token| !trace_shell_token_is_command_separator(token))
        .cloned()
        .collect::<Vec<_>>();

    mounts.iter().any(|mount| {
        mount.writable
            && is_privileged_docker_host_mount_path(&mount.host_path)
            && docker_container_command_writes_mount_path(&container_command, &mount.container_path)
    })
}

fn docker_run_args_start(tokens: &[String]) -> Option<usize> {
    let mut index = 0;
    while index < tokens.len() {
        match tokens[index].as_str() {
            "run" => return Some(index + 1),
            "container" if tokens.get(index + 1).map(String::as_str) == Some("run") => {
                return Some(index + 2);
            }
            token if token.starts_with('-') => {
                index += if docker_global_option_takes_value(token) {
                    2
                } else {
                    1
                };
...
fn parse_docker_volume_mount(spec: &str) -> Option<TraceDockerBindMount> {
    let mut parts = spec.split(':');
    let host_path = parts.next()?.trim();
    let container_path = parts.next()?.trim();
    if host_path.is_empty()
        || container_path.is_empty()
        || !docker_mount_source_is_host_path(host_path)
    {
        return None;
    }
    let writable = !parts.flat_map(|part| part.split(',')).any(|option| {
        let option = option.trim().to_ascii_lowercase();
        option == "ro" || option == "readonly"
    });
    Some(TraceDockerBindMount {
        host_path: host_path.to_string(),
        container_path: normalize_docker_mount_path(container_path),
        writable,
    })
}

fn parse_docker_key_value_mount(spec: &str) -> Option<TraceDockerBindMount> {
    ...
    for field in spec.split(',') {
        ...
        let (key, value) = field
            ...
            .unwrap_or_else(|| (field.to_ascii_lowercase(), ""));
        match key.as_str() {
            "type" => mount_type = Some(value.to_ascii_lowercase()),
            "src" | "source" => host_path = Some(value.to_string()),
            "dst" | "destination" | "target" => container_path = Some(value.to_string()),
            "readonly" | "read-only" | "ro" => {
                let value = value.to_ascii_lowercase();
                readonly = value.is_empty() || !matches!(value.as_str(), "0" | "false" | "no");
            }
            _ => {}
        }
    }
    ...
    let container_path = container_path?.trim().to_string();
    if host_path.is_empty()
        || container_path.is_empty()
        || !docker_mount_source_is_host_path(&host_path)
    {
...
fn docker_container_command_writes_mount_path(command: &[String], mount_path: &str) -> bool {
    if command.is_empty() {
        return false;
    }
    if docker_container_tokens_write_mount_path(command, mount_path) {
        return true;
    }
    docker_container_shell_script(command).is_some_and(|script| {
        shell_words_for_trace(script)
            .as_deref()
            .is_some_and(|tokens| docker_container_tokens_write_mount_path(tokens, mount_path))
    })
}

fn docker_container_tokens_write_mount_path(tokens: &[String], mount_path: &str) -> bool {
    let writes = tokens
        .iter()
        .any(|token| docker_container_token_is_write_operation(token));
    let touches_mount = tokens
        .iter()
        .any(|token| docker_container_token_targets_mount_path(token, mount_path));
    writes && touches_mount
}

fn docker_container_shell_script(command: &[String]) -> Option<&str> {
    let program = command.first()?;
    if !matches!(trace_command_basename(program), "sh" | "bash") {
        return None;
    }
    command
        .windows(2)
...
fn docker_container_token_is_write_operation(token: &str) -> bool {
    if matches!(token, ">" | ">>" | ">|") {
        return true;
    }
    matches!(
        trace_command_basename(token),
        "install"
            | "cp"
            | "mv"
            | "tee"
...
fn docker_container_token_targets_mount_path(token: &str, mount_path: &str) -> bool {
    let token = token
        .trim()
    ...
    path_is_at_or_under(token, mount_path)
}

fn docker_mount_source_is_host_path(path: &str) -> bool {
    path.starts_with('/') || path.starts_with("~/") || path.starts_with("$HOME/")
}

fn is_privileged_docker_host_mount_path(path: &str) -> bool {
    let path = path.to_ascii_lowercase();
    path == "/"
        || is_privileged_trace_path(&path)
...
fn docker_global_option_takes_value(token: &str) -> bool {
    ...
    matches!(
        token,
        "--config" | "--context" | "--host" | "-c" | "-H" | "--log-level"
    )
}

fn docker_run_option_takes_value(token: &str) -> bool {
    if token.contains('=') {
        return false;
    }
    matches!(
...
fn trace_shell_token_is_command_separator(token: &str) -> bool {
    matches!(token, "|" | "&&" | "||" | ";")
}

fn trace_command_basename(command: &str) -> &str {
    command
        .rsplit('/')
        .next()
        .unwrap_or(command)
...
fn normalize_docker_mount_path(path: &str) -> String {
...
fn path_is_at_or_under(path: &str, root: &str) -> bool {
    let path = normalize_docker_mount_path(path);
    let root = normalize_docker_mount_path(root);
...
mod tests {
    ...
    fn trace_prompt_reports_remote_script_execution_as_step() {
        ...
        assert!(prompt.contains("docker run"));
        assert!(prompt.contains("writable host bind mount"));
    ...
    #[test]
    fn trace_command_safety_detects_docker_privileged_host_mount_writes() {
        let command = "docker run --rm --pull=never -v /etc:/host-etc:rw ubuntu:22.04 /usr/bin/install -m 0644 -o 0 -g 0 /host-etc/sddm.conf.bak /host-etc/sddm.conf";
        let rating = trace_safety_rating_for_command(
            command,
            &[trace_step_with(
                "Writes container path `/host-etc/sddm.conf`.",
                "modify",
                Some("/host-etc/sddm.conf"),
                None,
            )],
        );

        assert_eq!(rating.level, "danger");
        assert_trace_safety_reason(&rating, "writes host files through Docker bind mount");
    }

    #[test]
    fn trace_docker_privileged_host_mount_detector_handles_mount_forms_and_counterexamples() {
        assert!(trace_command_writes_privileged_host_path_through_docker(
            "docker container run --mount type=bind,source=/etc,target=/host-etc ubuntu sh -c 'cp /host-etc/a /host-etc/b'"
        ));
        assert!(trace_command_writes_privileged_host_path_through_docker(
            "sudo /usr/bin/docker run --volume=/:/host-root alpine tee /host-root/etc/sudoers"
        ));
        assert!(!trace_command_writes_privileged_host_path_through_docker(
            "docker run -v /etc:/host-etc:ro ubuntu cp /host-etc/a /tmp/a"
        ));
        assert!(!trace_command_writes_privileged_host_path_through_docker(
            "docker run -v /tmp/project:/work:rw ubuntu install -m 0644 /work/a /work/b"
        ));
        assert!(!trace_command_writes_privileged_host_path_through_docker(
            "docker run -v /etc:/host-etc:rw ubuntu cat /host-etc/sddm.conf"
        ));
    }
