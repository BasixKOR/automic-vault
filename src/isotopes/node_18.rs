...
mod tests {
    ...
    fn detects_plaintext_registry_token() {
        let contents = "//registry.npmjs.org/:_authToken=npm_secret\n";

        assert!(npmrc_contains_plaintext_auth_token(contents));
    }
...
pub(crate) fn findings(home: &std::path::Path) -> Vec<crate::Finding> {
    super::radioisotope::findings("node@18", install_insecurity_reasons, home)
}
