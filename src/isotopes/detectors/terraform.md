...
keychain and wraps `terraform` so `TF_CLI_CONFIG_FILE` points at a temporary
non-secret config file that enables Terraform's native credentials helper
protocol. The helper itself is installed as
`~/.terraform.d/plugins/terraform-credentials-av` and delegates to
