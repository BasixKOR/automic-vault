...
mod tests {
    ...
    fn detects_secret_key() {
        assert!(config_contains_secret(
            r#"{"aliases":{"minio":{"accessKey":"fake","secretKey":"secret"}}}"#
        ));
    }

    #[test]
    fn detects_session_token() {
        assert!(config_contains_secret(
            r#"{"aliases":{"minio":{"sessionToken":"session"}}}"#
        ));
    }
    ...
    fn ignores_empty_secret_values() {
        assert!(!config_contains_secret(
            r#"{"aliases":{"minio":{"accessKey":"fake","secretKey":"","sessionToken":""}}}"#
        ));
    }
...
pub(crate) fn findings(home: &std::path::Path) -> Vec<crate::Finding> {
    super::radioisotope::findings("minio-mc", install_insecurity_reasons, home)
}
