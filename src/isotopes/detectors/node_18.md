...
This radioisotope modifies the Homebrew `node@18` package, but only changes
the installed `npm` launcher. `node` and `npx` continue to run without isotope
...
The post-install hook wraps `/opt/node@18/bin/npm`. The wrapper injects
`NODE_AUTH_TOKEN` only when an `npm publish` invocation is detected, then
execs the original npm launcher.
