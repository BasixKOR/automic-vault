...
pub fn install_insecurity_reasons() -> Result<Vec<String>, String> {
    let path = opentofu_credentials_path()?;
...
fn opentofu_credentials_path() -> Result<PathBuf, String> {
...
mod tests {
    ...
    fn detects_token() {
        assert!(credentials_file_contains_token(
            r#"{"credentials":{"app.opentofu.org":{"token":"secret"}}}"#
        ));
    }
    ...
    fn ignores_empty_token() {
        assert!(!credentials_file_contains_token(
            r#"{"credentials":{"app.opentofu.org":{"token":""}}}"#
        ));
    }
...
pub(crate) fn findings(home: &std::path::Path) -> Vec<crate::Finding> {
    super::radioisotope::findings("opentofu", install_insecurity_reasons, home)
}
