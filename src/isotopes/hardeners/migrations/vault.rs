...
const VAULT_TOKEN_ENV_KEY: &str = "VAULT_TOKEN";
...
pub fn migrate_credentials() -> Result<(), String> {
    migrate_credentials_file(&vault_token_path()?, &KeychainCredentialStore).map(|_| ())
}
...
pub fn migrate_credentials_file(path: &Path, store: &dyn CredentialStore) -> Result<bool, String> {
    ...
    store.store_secret(VAULT_TOKEN_ENV_KEY, token)?;
    fs::write(path, "").map_err(|err| format!("failed to write {}: {err}", path.display()))?;
    Ok(true)
}

fn vault_token_path() -> Result<PathBuf, String> {
    let home = std::env::var_os("HOME")
        .map(PathBuf::from)
        .ok_or_else(|| "HOME is not set".to_string())?;
    Ok(home.join(".vault-token"))
}
...
mod tests {
    ...
    #[test]
    fn migrates_vault_token() {
        let path = std::env::temp_dir().join(format!("vault-token-{}", std::process::id()));
        fs::write(&path, "hvs.secret\n").unwrap();
        ...
        assert_eq!(
            ...
            &[(VAULT_TOKEN_ENV_KEY.to_string(), "hvs.secret".to_string())]
        );
    ...
    fn keys_and_default_path_require_home() {
        assert_eq!(keys(), &[VAULT_TOKEN_ENV_KEY]);

        let previous_home = std::env::var_os("HOME");
        unsafe { std::env::remove_var("HOME") };
        let err = vault_token_path().unwrap_err();
    ...
    fn migrate_credentials_ignores_missing_and_blank_token_files() {
        ...
        let missing = temp.join(format!("vault-missing-{}", std::process::id()));
        let blank = temp.join(format!("vault-blank-{}", std::process::id()));
        let store = TestCredentialStore::default();
    ...
    fn migrate_credentials_preserves_file_when_store_fails() {
        let path = std::env::temp_dir().join(format!("vault-store-failure-{}", std::process::id()));
        fs::write(&path, "hvs.secret\n").unwrap();

        let err = migrate_credentials_file(&path, &FailingCredentialStore).unwrap_err();

        assert!(err.contains("store failed"));
        assert_eq!(fs::read_to_string(&path).unwrap(), "hvs.secret\n");
        fs::remove_file(path).unwrap();
    }
}
