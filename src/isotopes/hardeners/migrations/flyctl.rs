...
const FLY_ACCESS_TOKEN_ENV_KEY: &str = "FLY_ACCESS_TOKEN";
...
pub fn keys() -> &'static [&'static str] {
    &[FLY_ACCESS_TOKEN_ENV_KEY]
}

pub fn migrate_credentials() -> Result<(), String> {
    migrate_credentials_file(&fly_config_path()?, &KeychainCredentialStore).map(|_| ())
}
...
pub fn migrate_credentials_file(path: &Path, store: &dyn CredentialStore) -> Result<bool, String> {
    ...
    let Some(token) = fly_config_access_token(&contents) else {
        return Ok(false);
    };

    store.store_secret(FLY_ACCESS_TOKEN_ENV_KEY, &token)?;
...
fn remove_access_token_lines(contents: &str) -> String {
    let mut output = contents
        .lines()
        .filter(|line| !line.trim_start().starts_with("access_token:"))
        .collect::<Vec<_>>()
        .join("\n");
    output.push('\n');
    output
...
mod tests {
    ...
    #[test]
    fn migrates_fly_access_token() {
        let path = std::env::temp_dir().join(format!("fly-config-{}", std::process::id()));
        fs::write(&path, "access_token: FlyV1 secret\nmetrics: true\n").unwrap();
        ...
        assert_eq!(
            ...
            &[(
                FLY_ACCESS_TOKEN_ENV_KEY.to_string(),
                "FlyV1 secret".to_string()
            )]
        );
        assert_eq!(fs::read_to_string(&path).unwrap(), "metrics: true\n");
