...
3. `av-web`
   - A Rust HTTP service for the package catalog website origin on Atlas.
   - Serves `/pkg/` and localized package routes from private SQLite data, not
     from static `www/pkg` files.
   - Binds to `127.0.0.1:3004` in production.
...
- `src/web/` is the `av-web` Atlas package-origin service. It owns package page
  routing, rendering, markdown alternates, package sitemaps, package CSS/JS,
  and `/pkg/search.json`.
...
- The local artifact is generated by `scripts/generate-pkg-sqlite.py` and
  deployed with `scripts/deploy-pkg-origin.sh`.
- Do not port package work back to static `www/pkg/**` pages or Pagefind.
