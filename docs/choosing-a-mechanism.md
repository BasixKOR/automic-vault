...
| "My consumer reads credentials from file descriptors, including exact multiline bytes" | **FD delivery** (`av inject --mode=fd +NAME:3 -- COMMAND`) | One anonymous pipe per Secret, then EOF. Requires fresh Approval every time; Direct Access Rules and Blessings do not authorize it. See [FD delivery and limits](direct-secret-access.md#apply-secrets-through-file-descriptors). |
| "I need a secret in the environment via `av inject` and none of the above fits" | **Direct Secret Access** | Broadest and least preferred option: it grants one Verified Launcher access to one Secret Name across any Target and arguments it chooses, and that Target receives and controls the Secret. It does not authorize FD delivery, the separate Secret Disclosure operation, listing Secret Names, or Secret mutation. Prefer hardening, blessing, or the proxy first, see [Direct Secret Access](direct-secret-access.md#safer-alternatives). |
...
1. **Does this code already have a verifiable identity?** A signed app or
   ...
   separate Launcher Endorsement then names which Verified Launcher may run
   that Blessing automically.
