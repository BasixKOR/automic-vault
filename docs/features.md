...
| Secret scanner | Scans isotope detectors and likely local secret files for plaintext credentials, with human, JSON, JSONL, and `--isotopes-only` modes. | Finds credentials that are already visible to agents before they are used or leaked. |
