...
the installed intervention and its dependencies. An Isotope supplies an Automic
Vault-compatible build or wrapper where upstream behavior cannot support the
required boundary. Each Automic Vault-maintained Tool fork produces and
publishes its signed Isotope asset. The signed Isotopes Homebrew tap pins the
expected fork release URL and digest. A Hardener installs a tap Isotope through
its fully qualified formula when Homebrew is available unless a reviewed
Tool-specific installer owns its multi-file runtime. Without Homebrew,
executable-only Isotopes are verified against the same manifest and installed
directly into `/usr/local/bin`; Automic Vault then assumes responsibility for
their updates. Reviewed multi-file distributions use a verified, root-owned
package prefix under `/opt/av/<tool>`, as specified by [ADR
0031](adr/0031-isotope-installation-selection.md). The multi-file Wrangler
Isotope uses its tap formula as an update manifest and installs directly into
that protected prefix, as specified by [ADR
0042](adr/0042-wrangler-isotope-runtime.md).
